...
//   // Memoize stars to prevent recreation on re-renders
//   const stars = useMemo(() => {
//     if (!showStars || disabled) return [];
//     return Array.from({ length: starCount }, () => ({
//       x: useRef(new Animated.Value(Math.random() * (screenWidth * 0.3))).current,
//       y: useRef(new Animated.Value(20 + Math.random() * 80)).current,
//       opacity: useRef(new Animated.Value(0)).current,
//       scale: useRef(new Animated.Value(0.5)).current,
//     }));
//   }, [starCount, showStars, disabled]);
...
//         starOpacity: 0.6,
//         borderOpacity: [0.3, 0.6],
//       },
//       medium: {
//         glowDuration: 3000,
//         shadowOpacity: [0.3, 0.7],
//         starOpacity: 0.8,
//         borderOpacity: [0.4, 0.9],
//       },
//       high: {
//         glowDuration: 2000,
//         shadowOpacity: [0.4, 0.9],
//         starOpacity: 1,
//         borderOpacity: [0.5, 1],
//       },
//     };
...
//         animations.push(shine1Animation, shine2Animation, shine3Animation);
//       }

//       // Star animations
//       if (showStars && stars.length > 0) {
//         const starAnimations = stars.map((star, i) => {
//           const delay = i * 800;
//           const duration = 2000 + (i % 3) * 500;
//           const moveDistance = 3 + (i % 5);
//           const startX = star.x._value;
//           const startY = star.y._value;
...
//       }
//     } catch (error) {
//       console.warn('GlowWrapper animation error:', error);
//     }
...
//         });
//       } catch (error) {
//         console.warn('GlowWrapper cleanup error:', error);
//       }
//     };
//   }, [showStars, showShinePatches, stars?.length, disabled, intensityConfig.glowDuration]);

//   // Interpolations
//   const borderColor = disabled ? (isDarkMode ? 'rgba(139, 92, 246, 0.4)' : 'rgba(139, 92, 246, 0.3)') : glowAnim.interpolate({
//     inputRange: [0, 0.5, 1],
//     outputRange: [
//       isDarkMode 
//         ? `rgba(139, 92, 246, ${intensityConfig.borderOpacity[0]})` 
//         : `rgba(139, 92, 246, ${intensityConfig.borderOpacity[0] * 0.7})`,
//       isDarkMode 
//         ? `rgba(139, 92, 246, ${intensityConfig.borderOpacity[1]})` 
//         : `rgba(139, 92, 246, ${intensityConfig.borderOpacity[1] * 0.8})`,
//       isDarkMode 
//         ? `rgba(139, 92, 246, ${intensityConfig.borderOpacity[0]})` 
//         : `rgba(139, 92, 246, ${intensityConfig.borderOpacity[0] * 0.7})`,
//     ],
...
//   const shadowOpacity = disabled ? 0.3 : glowAnim.interpolate({
//     inputRange: [0, 1],
...
//       {/* Animated Stars */}
//       {showStars && !disabled && stars.map((star, i) => (
//         <Animated.View
//           key={`star-${i}`}
//           style={[
//             styles.star,
...
const GlowWrapper = memo(({
  ...
  const borderColor = useMemo(() => {
    if (disabled) {
      return isDarkMode ? 'rgba(139, 92, 246, 0.4)' : 'rgba(139, 92, 246, 0.3)';
    }
  ...
  }, [disabled, isDarkMode, glowAnim, intensityConfig.borderOpacity]);
