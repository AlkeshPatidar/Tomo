...
const OtherUserDetail = ({navigation, route}) => {
  ...
  useEffect(() => {
    fetchData()
    fetchMyPost()
    fetchSavedPosts() // Add this function later
  }, [isFocused])
  ...
  const renderPostsGrid = () => {
    const currentPosts = activeTab === 'all' ? allPosts : savedPosts

    if (currentPosts.length === 0) {
      return (
        <View style={styles.emptyState}>
          <Text style={styles.emptyStateText}>
            {activeTab === 'all' ? 'No posts yet' : 'No saved posts'}
          </Text>
        </View>
      )
    }
